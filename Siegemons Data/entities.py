...
class WeaponConstructor(object):
    ...
    def __call__(self, weapon, name, strength, durability):
        if not self.dropped_shield:
            ...
            return MeleeWeapon("iRAT", "Cave Rat", 2, ITEM_DURABILITIES['Flimsy Shield'])
        elif not self.dropped_bow:
            self.dropped_bow = True
            return RangedWeapon("iRAT", "Cave Rat", 4, ITEM_DURABILITIES['Slimy Bow'])
        elif not self.dropped_sword:
            self.dropped_sword = True
            return MeleeWeapon("iRAT", "Cave Rat", 2, ITEM_DURABILITIES['Knife'])
        else:
...
class Enemy(Entity):
    ...
    def __init__(self, image, name, health, item, EXP):
        self.image = image
        self.name = name
        self.health = health
        self.item = item
