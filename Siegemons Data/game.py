...
class Game(object):
    ...
    def printItems(self):  # TODO: list enemy weapon in case we want it?
        ...
        main_hand_stats = ["Name: " + str(main_hand.name), "Level: " + str(main_hand.strength), 'Health: ' + str(main_hand.durability), 'EXP: ' + str(main_hand.EXP)] if main_hand else None
        offhand_stats = ["Name: " + str(offhand.name), "Level: " + str(offhand.strength), 'Health: ' + str(offhand.durability)] if offhand else None
        ...
        for item in self.inventory.get_pack():
            items.append(["Name: " + item['name'], "Level: " + str(item['strength']), 'Health: ' + str(item['durability']), 'EXP: ' + str(item['EXP'])])
        ...
        lines.append("| {0:34s}".format('Active Siegemon' if main_hand else (offhandText if offhand else '1.')) +
    ...
    def all_Siegemons_Dead(self):
        dead = True
        for i in self.inventory.pack:
            if i.durability > 0:
                dead = False
        if self.inventory.main_hand.durability > 0:
            dead = False
        return dead
    ...
    def playerTurn(self):
        ...
        if decisions[0] == '1' or decisions[0] == '2':
            items = self.inventory.get_pack()
            if len(items) == 0:
                self.messages.append("You have no Siegemons to switch to!")
                return False
            
            handChoice = 'Main Hand'
            self.swap_weapon_to = handChoice
            
            # Print the weapons available to swap to
            validSwap = False
            while not validSwap:
                self.messages.append('Which Siegemon do you want to switch to?'.format(handChoice))
                itemIndexOffset = 1
                if self.inventory.get_main_hand():
                    itemIndexOffset += 1
                if self.inventory.get_offhand():
                ...
                for i in range(len(items)):
                    ...
                    weaponMssg = '{0} (Type: {1}, Level: {2}, Health: {3})'.format(item['name'], item['type'], item['strength'], item['durability'])
                ...
                try:
                    ...
                    if not self.inventory.is_valid_item_index(itemChoice):
                        ...
                        self.messages.append('Go, {0}!'.format(self.inventory.main_hand.name))
                        validSwap = True
                    else:
        ...
        # ATTACKING
            ...
            self.messages.append("Fireballs aren't allowed in Siegemons, dummy!")
            '''
            ...
            if self.inventory.main_hand.durability == 0:
                self.messages.append("Your Siegemon has fainted! You must switch to another Siegemon.")
                return False
            ...
            self.messages.append(
                "Your Siegemon {0}s the {1} for {2} damage!".format(
                    playerAction,
        ...
        # ESCAPE
            ...
            if True:
                ...
                if not will_escape:
                    self.messages.append("You failed to run away!")
                else:
                    self.messages.append("You fled from battle!")
                    self.printScreen()
        ...
        # ITEMS
            ...
            if result:
                ...
                self.inventory.main_hand.durability = min(
                    100 +
                    self.inventory.main_hand.durability,
                    self.inventory.main_hand.max_durability)
                self.messages.append(
                    "Your Siegemon drank a potion and recovered 100 health!")
                self.printScreen()
                return True
            else:
        ...
        # SHIELDING
            ...
            self.messages.append("Your Siegemon raised its shield!")
            return True
            '''
            else:
                self.messages.append(
    ...
    def enemyTurn(self):
        ...
        if self.playerStance == "DEFENSIVE" and (random.uniform(0, 1) + self.inventory.main_hand.strength * 0.01) >= 0.25: #self.seerdata.current_block:
            self.messages.append("It successfully blocked the enemy attack!")
        else:
            self.inventory.main_hand.durability -= damage
            if (self.inventory.main_hand.durability < 0):
                self.inventory.main_hand.durability = 0
            damageType = "hit" if not isinstance(
                self.current_enemy.item,
                RangedWeapon) else "shoot"
            self.messages.append(
                "The {0} {2}s your Siegemon for {1} damage!".format(
                    self.current_enemy.name,
                    damage,
                    damageType))
        return True
    ...
    def runEvent(self):
        ...
        while not self.current_enemy.isDead() and not self.all_Siegemons_Dead():
            self.printScreen()
        ...
        if self.current_enemy.isDead():
            ...
            if not self.current_enemy.name == "Evil Steve-n-Dorf":
                self.messages.append("Your Siegemon gained {0} EXP.".format(self.current_enemy.EXP))
                i = 1
                while (i * i <= (self.inventory.main_hand.EXP + self.current_enemy.EXP)):
                    i += 1
                i -= 1
                EXP_before = self.inventory.main_hand.EXP
                self.inventory.main_hand.EXP += self.current_enemy.EXP
                levels_gained = 0
                if (EXP_before < i * i):
                    self.messages.append("Your Siegemon leveled up!")
                    j = 1
                    while (j * j <= EXP_before):
                        j += 1
                    while (self.inventory.main_hand.EXP >= j * j):
                        levels_gained += 1
                        j += 1
                    self.inventory.main_hand.strength += levels_gained
                    self.inventory.main_hand.max_durability += 5
                    self.inventory.main_hand.durability += 1
                
                self.items_dropped += 1
                self.messages.append(
                    "Would you like to add the {} to your party?".format(self.current_enemy.name))
                self.current_enemy.image = "BLANK_ENEMY"

                items = self.inventory.get_pack()
                pickupMssg = '[1: Replace current Siegemon]'
                validOptions = ['1',]
                toAppend = ''
                for i in range(len(items)):
        ...
        elif self.all_Siegemons_Dead():
            self.printScreen()
            if not USE_AI: time.sleep(2)
            raise Defeat("All of your Siegemons have fainted.")
